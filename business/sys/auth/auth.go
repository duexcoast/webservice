...
// KeyLookup declares a method set of behavior for looking up private and public
// keys for JWT use.
...
// user claims and recreate the claims by parsing the token.
type Auth struct {
	activeKID string
	KeyLookup KeyLookup
...
func New(activeKID string, keyLookup KeyLookup) (*Auth, error) {
	...
	// The activeKID represents the private key used to sign new toekns.
...
func (a *Auth) ValidateToken(tokenStr string) (Claims, error) {
	...
	// since we're using RegisteredClaims, we might need to pass in a non-pointer
	// version of claims OR, allocate the proper memory before hand, to avoid
	// a panic.
