...
// Open kows how to open a database connection based on the configuration.
...
// StatusCheck returns nil if it can successfullyl talk to the database. It
// returns a non-nil error otherwise.
func StatusCheck(ctx context.Context, db *sqlx.DB) error {

	// First check that we can ping the database.
	var pingError error
	for attempts := 1; ; attempts++ {
		...
		// We won't loop forever because the context will eventually timeout.
		if ctx.Err() != nil {
			return ctx.Err()
		}
	}
...
// collection of data to be unmarshaled into the slice.
